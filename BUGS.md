...
| 7 | 2026-04-29 | not yet validated | unknown | `core.proxy.mitmproxy_mode` + flow logger | The mitmproxy-flow-logger.py addon writes one NDJSON line per response and `mitmproxy_flow_source` tails the file. Wired end-to-end but **not validated against real traffic** in P2.10b commit (user was on a podcast and skipped the live test). | **open** — re-validate by generating HTTPS traffic from victim VM after next router up cycle. |
