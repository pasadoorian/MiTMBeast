...
| 4 | 2026-04-29 | user feedback | minor (UX) | TUI Dashboard log pane | Pane uses a `Static` widget capped at 12 lines — output during `up`/`down` scrolls past too fast to read | **fixed** (`3513dac`) — replaced with `RichLog` (`max_lines=5000`, `auto_scroll`, scrollback retained). |
